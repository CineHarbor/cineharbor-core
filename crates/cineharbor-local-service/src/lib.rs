...
const LOCAL_SERVICE_ACCESS_TOKEN_HEADER: &str = "x-cineharbor-local-token";
const LOCAL_SERVICE_ADMIN_CAPABILITY_HEADER: &str = "x-cineharbor-admin-capability";
const DEFAULT_CONFIG_FILE_NAME: &str = "config.example.json";
const DEFAULT_DATA_DIR_NAME: &str = ".cineharbor-desktop";
const DEFAULT_SQLITE_FILE_NAME: &str = "cineharbor-desktop.sqlite3";
...
const DEFAULT_SITE_NAME: &str = "CineHarbor";
...
pub async fn run(cli: Cli) -> Result<()> {
    ...
    if effective_bind_host(&state.host) != state.host {
        info!(
            "CineHarbor local service listening on {} with public base URL {} and config {}",
            listener_addr,
        ...
        info!(
            "CineHarbor local service listening on {} with config {}",
            listener_addr,
            state.config_path.display()
        );
    }
...
fn apply_cors_headers_for_origin(headers: &mut HeaderMap, request_origin: Option<&str>) {
    ...
    headers.insert(
        ...
        HeaderValue::from_static(
            "Content-Type, Range, Origin, Accept, Authorization, X-CineHarbor-Response-Status, X-CineHarbor-Download-Intent, X-CineHarbor-Local-Token, X-CineHarbor-Admin-Capability",
        ),
    );
    headers.insert(
...
async fn export_admin_data_migration(
    ...
    build_binary_file_response(
        encrypted,
        &format!("cineharbor-backup-{}.dat", current_timestamp_ms()),
    )
}
...
mod tests {
    ...
    async fn health_route_returns_ok() {
        ...
        let state = AppState::new(
            ...
            temp_dir.path.join("data/cineharbor.sqlite3"),
        );
    ...
    fn legacy_admin_persistence_is_migrated_and_sqlite_remains_authoritative() {
        ...
        write_test_admin_persistence(
            ...
            json!({
                "config": {
                    "SiteConfig": {
                        "SiteName": "Stale Legacy CineHarbor"
                    }
                }
            }),
        );
    ...
    async fn cors_preflight_allows_download_intent_header() {
        ...
        assert!(
            allow_headers
                .to_ascii_lowercase()
                .contains("x-cineharbor-download-intent"),
            "expected allow headers to include x-cineharbor-download-intent, got: {allow_headers}"
        );
    }
    ...
    async fn configured_local_service_rejects_api_requests_without_access_token() {
        ...
        let authorized_download_runtime = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/download-runtime/tasks")
                    .header("X-CineHarbor-Local-Token", "test-access-token")
                    .body(Body::empty())
            ...
            let authorized = app
                ...
                .oneshot(
                    Request::builder()
                        ...
                        .uri(path)
                        .header("X-CineHarbor-Local-Token", "test-access-token")
                        .header("X-CineHarbor-Admin-Capability", "test-admin-capability")
                        .body(Body::empty())
...
"#;
    ...
    async fn profile_bootstrap_endpoint_returns_runtime_sync_and_local_auth_snapshot() {
        ...
        let raw_config = json!({
          ...
          "site_name": "Bootstrap CineHarbor",
    ...
    async fn admin_data_migration_import_route_preserves_local_identity_layer() {
        ...
        admin_config.site_config.site_name = "Imported CineHarbor".to_string();
        ...
        let boundary = "----CineHarborBoundary";
    ...
    async fn refresh_admin_config_subscription_if_due_updates_local_state() {
        ...
        assert!(raw_config.contains("Updated CineHarbor"));
    ...
    async fn profile_outbox_worker_pushes_only_the_due_head_and_exposes_pending_status() {
        ...
        store
            .apply_local_mutation_and_enqueue(
                "remote-user",
                &device_id,
                cineharbor_profile::ProfileDomain::Favorites,
                &std::collections::BTreeMap::from([("demo+1".to_string(), favorite.clone())]),
                cineharbor_profile::ProfileMutation::Upsert {
        ...
        store
            .apply_local_mutation_and_enqueue(
                ...
                cineharbor_profile::ProfileMutation::Delete {
                    entity_key: "demo+1".to_string(),
                },
            )
    ...
    async fn profile_sync_status_survives_sidecar_restart_with_latest_auth_blocked_profile() {
        ...
        let data_dir = temp_dir.path.join("data");
        let sqlite_path = data_dir.join("cineharbor.sqlite3");
    ...
    async fn live_proxy_m3u8_endpoint_rewrites_proxy_urls() {
        ...
        let service_url = format!(
            "/api/proxy/m3u8?cineharbor-source=news&url={}",
        ...
        assert!(manifest.contains("cineharbor-source=news"));
    ...
    async fn live_precheck_endpoint_detects_mp4_streams() {
        ...
        let service_url = format!(
            "/api/live/precheck?cineharbor-source=news&url={}",
    ...
    async fn download_runtime_task_snapshot_persists_across_state_restart() {
        ...
        let data_dir = temp_dir.path.join("data");
        let sqlite_path = temp_dir.path.join("data/cineharbor.sqlite3");
        let app = build_router(AppState::new(
            DEFAULT_HOST.to_string(),
            DEFAULT_PORT,
            config_path.clone(),
    ...
    impl TestDir {
        fn new() -> Self {
            ...
            let path = env::temp_dir().join(format!(
                "cineharbor-local-service-tests-{}-{}",
                std::process::id(),
