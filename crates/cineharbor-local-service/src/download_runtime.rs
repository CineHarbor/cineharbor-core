...
const DOWNLOAD_MANIFEST_REQUEST_INTENT_HEADER: &str = "x-cineharbor-download-intent";
...
fn parse_download_manifest_proxy_request(
    ...
    let parsed_url = Url::parse(request_url)
        .ok()
        .or_else(|| Url::parse(&format!("http://cineharbor.local{request_url}")).ok())?;
...
fn parse_download_runtime_status(headers: &HeaderMap) -> AppResult<StatusCode> {
    let raw_status = headers
        .get(HeaderName::from_static("x-cineharbor-response-status"))
        .and_then(|value| value.to_str().ok())
        .unwrap_or("200")
        .trim();
