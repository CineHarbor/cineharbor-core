...
pub(crate) async fn commit_profile_sync_session_if_current(
    state: &AppState,
    session: cineharbor_sync::ProfileSyncSession,
    request_epoch: u64,
) -> bool {
...
pub(crate) async fn profile_sync_session_snapshot(
    state: &AppState,
) -> (Option<cineharbor_sync::ProfileSyncSession>, u64) {
...
mod tests {
    ...
    use cineharbor_sync::ProfileSyncSession;
    ...
    async fn stale_responses_cannot_pollute_authoritative_sync_cookie() {
        ...
        let stale_401 = state
            ...
            .forward_passthrough(
                Some(&upstream.base_url),
                cineharbor_sync::ProfileSyncForwardRequest::get("/api/stale-401"),
            )
            .await
    ...
    async fn login_outcome(
        ...
        base_url: &str,
        username: &str,
    ) -> cineharbor_sync::ProfileSyncForwardOutcome {
        state
            .profile_sync
            .forward_login(
                Some(base_url),
    ...
    async fn cookie_echo(state: &AppState, base_url: &str) -> String {
        let response = state
            ...
            .forward(
                Some(base_url),
                cineharbor_sync::ProfileSyncForwardRequest::get("/api/cookie"),
            )
            .await
            .expect("cookie echo");
    ...
    impl TestDir {
        fn new() -> Self {
            ...
            let path = env::temp_dir().join(format!("cineharbor-profile-sync-epoch-test-{unique}"));
        ...
        fn state(&self) -> AppState {
            AppState::new(
                ...
                self.path.join("data"),
                self.path.join("data/cineharbor.sqlite3"),
            )
        }
    }
